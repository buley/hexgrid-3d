...
describe('Quaternion': unknown, (: unknown) => {
  describe('Static Constructors', () => {
    ...
    it('creates from axis-angle': unknown, (: unknown) => {
      const axis = new Vector3(0, 1, 0);
      const q = Quaternion.fromAxisAngle(axis, Math.PI / 2);
      expect(q.magnitude()).toBeCloseTo(1);
    });

    it('creates from Euler angles': unknown, (: unknown) => {
      const q = Quaternion.fromEuler(0, Math.PI / 2, 0);
      expect(q.magnitude()).toBeCloseTo(1);
    });

    it('creates from Euler degrees': unknown, (: unknown) => {
      const q = Quaternion.fromEulerDegrees(0, 90, 0);
      expect(q.magnitude()).toBeCloseTo(1);
    });

    it('creates from-to rotation': unknown, (: unknown) => {
    ...
    it('handles parallel vectors in fromToRotation': unknown, (: unknown) => {
      const from = new Vector3(1, 0, 0);
    ...
    it('handles opposite vectors in fromToRotation': unknown, (: unknown) => {
      const from = new Vector3(1, 0, 0);
      const to = new Vector3(-1, 0, 0);
      const q = Quaternion.fromToRotation(from, to);
      expect(q.magnitude()).toBeCloseTo(1);
    ...
    it('creates from rotation matrix': unknown, (: unknown) => {
      const m = Matrix4.rotationY(Math.PI / 4);
      const q = Quaternion.fromMatrix(m);
      expect(q.magnitude()).toBeGreaterThan(0);
    });

    it('creates random quaternion': unknown, (: unknown) => {
      const q = Quaternion.random();
      expect(q.magnitude()).toBeCloseTo(1);
    });

    it('creates look rotation': unknown, (: unknown) => {
  ...
  describe('Basic Operations': unknown, (: unknown) => {
    ...
    it('sets values': unknown, (: unknown) => {
    ...
    it('copies quaternion': unknown, (: unknown) => {
      const q1 = new Quaternion(1, 2, 3, 4);
    ...
    it('multiplies quaternions': unknown, (: unknown) => {
    ...
    it('premultiplies quaternions': unknown, (: unknown) => {
      const q1 = Quaternion.fromAxisAngle(new Vector3(0, 1, 0), Math.PI / 4);
    ...
    it('adds quaternions': unknown, (: unknown) => {
    ...
    it('scales quaternion': unknown, (: unknown) => {
    ...
    it('calculates dot product': unknown, (: unknown) => {
  ...
  describe('Normalization & Magnitude': unknown, (: unknown) => {
    ...
    it('normalizes quaternion': unknown, (: unknown) => {
      const q = new Quaternion(2, 0, 0, 0);
      const n = q.normalize();
      expect(n.magnitude()).toBeCloseTo(1);
    });

    it('handles normalizing zero quaternion': unknown, (: unknown) => {
  ...
  describe('Inverse & Conjugate': unknown, (: unknown) => {
    ...
    it('calculates inverse': unknown, (: unknown) => {
    ...
    it('handles inverse of zero quaternion': unknown, (: unknown) => {
      const q = new Quaternion(0, 0, 0, 0);
      const inv = q.inverse();
  ...
  describe('Interpolation': unknown, (: unknown) => {
    ...
    it('handles slerp with negative dot product': unknown, (: unknown) => {
    ...
    it('handles slerp with very close quaternions': unknown, (: unknown) => {
      const q1 = Quaternion.identity();
    ...
    it('lerps between quaternions': unknown, (: unknown) => {
      const q1 = new Quaternion(0, 0, 0, 1);
    ...
    it('nlerps between quaternions': unknown, (: unknown) => {
      const q1 = Quaternion.identity();
      const q2 = Quaternion.fromAxisAngle(new Vector3(0, 1, 0), Math.PI / 2);
    ...
    it('squad interpolates through keyframes': unknown, (: unknown) => {
  ...
  describe('Rotation Operations': unknown, (: unknown) => {
    ...
    it('gets rotation angle': unknown, (: unknown) => {
      const q = Quaternion.fromAxisAngle(new Vector3(0, 1, 0), Math.PI / 2);
      expect(q.getAngle()).toBeCloseTo(Math.PI / 2);
    });

    it('gets rotation axis': unknown, (: unknown) => {
      const q = Quaternion.fromAxisAngle(new Vector3(0, 1, 0), Math.PI / 2);
      const axis = q.getAxis();
      expect(axis.y).toBeCloseTo(1);
    });

    it('handles zero rotation axis': unknown, (: unknown) => {
      const q = Quaternion.identity();
      const axis = q.getAxis();
      expect(axis.magnitude()).toBeCloseTo(1);
    });

    it('converts to axis-angle': unknown, (: unknown) => {
      const q = Quaternion.fromAxisAngle(new Vector3(0, 1, 0), Math.PI / 2);
    ...
    it('converts to Euler angles': unknown, (: unknown) => {
    ...
    it('converts to Euler degrees': unknown, (: unknown) => {
    ...
    it('converts to Matrix4': unknown, (: unknown) => {
      const q = Quaternion.fromAxisAngle(new Vector3(0, 1, 0), Math.PI / 2);
      const m = q.toMatrix4();
      expect(m.elements[0]).toBeCloseTo(0);
    });

    it('converts to Matrix3': unknown, (: unknown) => {
      const q = Quaternion.fromAxisAngle(new Vector3(0, 1, 0), Math.PI / 2);
      const m = q.toMatrix3();
      expect(m.elements[0]).toBeCloseTo(0);
    });
  ...
  describe('Utility Methods': unknown, (: unknown) => {
    ...
    it('checks identity': unknown, (: unknown) => {
    ...
    it('calculates exponential map': unknown, (: unknown) => {
      const v = new Vector3(0, Math.PI / 4, 0);
      const q = Quaternion.exp(v);
      expect(q.magnitude()).toBeCloseTo(1);
    });

    it('handles exp of zero vector': unknown, (: unknown) => {
      const v = Vector3.zero();
      const q = Quaternion.exp(v);
      expect(q.isIdentity()).toBe(true);
    });

    it('calculates logarithm': unknown, (: unknown) => {
      const q = Quaternion.fromAxisAngle(new Vector3(0, 1, 0), Math.PI / 4);
      const v = q.log();
      expect(v.magnitude()).toBeCloseTo(Math.PI / 4);
    });

    it('handles log of identity': unknown, (: unknown) => {
      const q = Quaternion.identity();
      const v = q.log();
      expect(v.magnitude()).toBeCloseTo(0);
    });

    it('calculates power': unknown, (: unknown) => {
      const q = Quaternion.fromAxisAngle(new Vector3(0, 1, 0), Math.PI / 2);
      const half = q.pow(0.5);
      expect(half.getAngle()).toBeCloseTo(Math.PI / 4);
    });

    it('handles power of identity': unknown, (: unknown) => {
      const q = Quaternion.identity();
      const result = q.pow(2);
      expect(result.isIdentity()).toBe(true);
    });

    it('converts to array': unknown, (: unknown) => {
      const q = new Quaternion(1, 2, 3, 4);
      const arr = q.toArray();
...
describe('DualQuaternion': unknown, (: unknown) => {
  ...
  it('clones dual quaternion': unknown, (: unknown) => {
  ...
  it('multiplies dual quaternions': unknown, (: unknown) => {
    const dq1 = DualQuaternion.fromRotationTranslation(
      Quaternion.identity(),
      new Vector3(1, 0, 0)
    );
  ...
  it('calculates conjugate': unknown, (: unknown) => {
  ...
  it('normalizes dual quaternion': unknown, (: unknown) => {
    const dq = DualQuaternion.fromRotationTranslation(
      Quaternion.identity(),
      new Vector3(1, 2, 3)
    );
  ...
  it('transforms point': unknown, (: unknown) => {
    const dq = DualQuaternion.fromRotationTranslation(
      Quaternion.identity(),
      new Vector3(1, 2, 3)
    );
  ...
  it('extracts rotation and translation': unknown, (: unknown) => {
    const rotation = Quaternion.fromAxisAngle(
      new Vector3(0, 1, 0),
      Math.PI / 4
    );
  ...
  it('sclerp interpolates dual quaternions': unknown, (: unknown) => {
    const dq1 = DualQuaternion.fromRotationTranslation(
      Quaternion.identity(),
      new Vector3(0, 0, 0)
    );
  ...
  it('handles power of pure translation': unknown, (: unknown) => {
    const dq = DualQuaternion.fromRotationTranslation(
      Quaternion.identity(),
      new Vector3(10, 0, 0)
    );
